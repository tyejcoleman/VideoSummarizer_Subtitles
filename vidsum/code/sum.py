...
def create_summary(filename, trimmed_video, regions):
    """ Join segments
    ...
    for (start, end) in regions:
        subclip = trimmed_video.subclip(start, end)
        subclips.append(subclip)
        last_end = end
    return concatenate_videoclips(subclips)


def get_summary(filename, trimmed_video, subtitles="1.srt"):
    """ Abstract function
    ...
    summary = create_summary(filename, trimmed_video, regions)
...
def trim_video(video_filenme, start, end):
    input_video = VideoFileClip(video_filenme)
    trimmed_video = input_video.subclip(start, end)
    return trimmed_video

def get_srt_file(trimmed_video):
    # need to return srt file
    return True
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--begin-time', help="Start time to trim")
    parser.add_argument('-e', '--end-time', help="End time to trim")
    ...
    trimmed_video = trim_video(args.video_file, args.begin_time, args.end_time)

    # Write trimmed file to make sure it worked
    # trimmed_video.to_videofile("finish.mp4", codec="libx264",temp_audiofile="temp.m4a", remove_temp=True, audio_codec="aac")

    srt_file = get_srt_file(trimmed_video)

    if not url:
        # proceed with general summarization
        get_summary(args.video_file, trimmed_video, args.subtitles_file)
